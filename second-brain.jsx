...
/* ═══════════════════════════════════════════════════════════════
   SECOND BRAIN — Layer 1: The Outliner Instrument
   ═══════════════════════════════════════════════════════════════ */
...
function findInTree(nodes, id, parent = null) {
...
function insertAfterInTree(nodes, targetId, newNode) {
  const result = [];
  for (const n of nodes) {
...
function insertAsChild(nodes, parentId, newNode) {
  return nodes.map((n) =>
    n.id === parentId
...
function insertBeforeInTree(nodes, targetId, newNode) {
  const result = [];
  for (const n of nodes) {
    if (n.id === targetId) result.push(newNode);
...
function updateText(nodes, id, text) {
  return nodes.map((n) =>
...
function updateNodeProp(nodes, id, key, value) {
  return nodes.map((n) =>
...
const HIGHLIGHTS = [null, "#69F0AE", "#FFD740", "#EA80FC"];
const PRIORITIES = [null, "low", "med", "high"];
const PRI_COLORS = { low: "#03DAC6", med: "#FFD740", high: "#CF6679" };
const PRI_LABELS = { low: "!", med: "!!", high: "!!!" };
...
function matchesFilter(node, query, filters) {
  let pass = true;
  if (query) {
...
function getZoomedNodes(nodes, zoomStack) {
  let current = nodes;
  for (const zid of zoomStack) {
    const found = findInTree(current, zid);
    if (found) {
...
function nodesToText(nodes, selectedIds, depth = 0) {
  let text = "";
  for (const n of nodes) {
...
function nodesToTextAll(node, depth) {
...
// OPML export
function toOPML(nodes, title = "Second Brain") {
...
// ─── Storage Abstraction ─────────────────────────────────────
// Swap this one object to move from artifact → Supabase → any backend

const db = {
  // Current: Claude artifact window.storage
  // Production: Supabase client.from('documents').upsert(...)
  async save(key, data) {
    try {
...
const USER_KEY = "second-brain-user";
